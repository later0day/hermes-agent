...
def build_classifier_messages(
    ...
    system = (
        ...
        "- 如果消息明显跨多个领域，可以放多个成员。\n"
        "- 如果消息是明确对【全体成员】发起的问候、集合指令或要求逐一回应"
        "（例如\"大家好\"\"所有人介绍一下自己\"\"请每位成员都分别做一下自我"
        "介绍\"\"everyone please respond\"\"hi all\"\"@all\"\"@所有人\"），"
        "members 请列出上面roster中的**全部**成员名字（一个不落），"
        "reason 里说明这是面向全员的广播——这属于真实匹配，不是空数组。\n"
        "- 如果消息明确不属于以上任何一人的职责范围（例如需要决策、协调、"
        "或纯闲聊寒暄，且不是面向全员的广播），members 请给空数组 []，并在 "
        "reason 里说明为什么不属于任何人 —— 不要为了凑数硬塞一个不相关的成员，"
        "这个判断本身就是有价值的信号，会被交给更合适的角色处理，不是错误答案。\n"
