...
def build_source_binding_key(
    ...
    """Build a binding key using the exact session-source identity rules.

    Delegate to ``build_session_key`` so binding identity stays aligned with
    session identity for Slack workspace scope, WhatsApp canonicalization,
    prospective threads, and DM participant fallback. Only the namespace
    differs: ``agent:main:...`` becomes ``source:...``.
    """
