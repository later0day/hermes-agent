...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if origin and origin.get("platform") == platform_name:
        # Upstream (#0c07a1975): bare-platform delivery prefers the configured
        # home target when set, else falls back to the origin chat.
        chat_id = _get_home_target_chat_id(platform_name)
        if chat_id:
            home_target = {
                "platform": platform_name,
            ...
            # Fork: carry origin chat_type so DingTalk conversation routing
            # (1:1 vs group) resolves correctly on the home target too.
            if origin.get("chat_type"):
                home_target["chat_type"] = origin.get("chat_type")
            return home_target
        target = {
            "platform": platform_name,
            "chat_id": str(origin["chat_id"]),
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        # Fork: build DingTalk-specific send metadata (conversation_type,
        # session_webhook) before the delivery dispatch.
        send_metadata = {}
        if thread_id:
